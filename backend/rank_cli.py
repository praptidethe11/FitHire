...
async def run_cli(candidates_path: str, jd_path: str, out_path: str, top_n: int, use_ai: bool,
    ...
    if not HAS_TRANSFORMERS and not allow_heuristic_fallback:
        print(
            ...
            "available in this environment — see the [WARN] above for why.\n"
            "        This run would NOT match a properly tested submission.\n"
            "        Fix: run `python download_models.py` once to cache the "
            "models, then re-run this command.\n"
            "        (Or pass --allow-heuristic-fallback to proceed anyway — "
            "dev/exploration only, not for your real submission CSV.)"
        )
        sys.exit(1)
...
# 1. Filter: Identify Top 5,000 candidates by BM25 score
    ...
    if HAS_TRANSFORMERS:
        ...
        import contextlib
        infer_context = contextlib.nullcontext()

    with infer_context:
        semantic_scores = precompute_semantic_scores(jd_text, top_candidates, deadline=SEMANTIC_BUDGET_DEADLINE)
        print(f"    Semantic scores precomputed in {time.time() - t_sem_start:.1f}s")
        
        for idx, sem_score in zip(top_indices, semantic_scores):
            c = candidates[idx]
            final, info = score_candidate(c, bm25_scores[idx], 1.0, jd_analysis, jd_text, jd_weights=jd_weights, semantic_score=sem_score)
    ...
    for idx in remaining_indices:
        c = candidates[idx]
        # Hardened schema initialization to guarantee safety during reporting, printing, or build_reasoning lookups
        info = {
            "candidate_id": c["candidate_id"],
            "final_score": 0.1,
            "name": c.get("profile", {}).get("anonymized_name", "Unknown"),
            "current_title": c.get("current_title") or c.get("profile", {}).get("current_title") or "Unknown",
            "signals": {},
            "breakdown": {
                "technical_fit": 0.1, "career_fit": 0.1, "recruiter_fit": 0.1,
                "skill_alignment": 0.1, "experience_fit": 0.1, "career_progression": 0.1,
                "notice_period": 0.1, "semantic_match": 0.1, "production_signals": 0.1,
                "company_fit": 0.1, "bm25_match": 0.1, "github_oss": 0.1,
                "title_relevance": 0.1, "certifications": 0.1, "engagement": 0.1
            }
        }
...
def main():
    ...
    parser.add_argument("--allow-heuristic-fallback", action="store_true",
                         help="Proceed even if sentence-transformers models aren't cached "
                              "locally (falls back to a weaker token-overlap heuristic for "
                              "semantic scoring). DEV/EXPLORATION ONLY — do not use this for "
                              "your real submission CSV; run `python download_models.py` "
                              "instead so the run matches what you actually tested.")
